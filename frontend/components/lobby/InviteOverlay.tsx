...
const InvitedPlayer = observer(({ player }: { player: InvitationMetadata }) => <Player>
    <Ionicons name={{
        Pending: "ios-more",
        Kicked: "md-close",
        Declined: "md-close",
        Accepted: "md-checkmark"
    }[player.state] || "ios-more"} size={20} color="white"/>
    <Name>{player.toSummoner.displayName}</Name>
</Player>);

const InvitablePlayer = ({ suggestion }: { suggestion: InvitationSuggestion }) => <TouchableOpacity
    onPress={() => lobby.invitePlayer(suggestion.summonerId)}>
    <Player>
...
const InviteOverlayContent = observer(() => {
    return <>
        ...
        <ScrollView alwaysBounceVertical={false} style={{ flex: 1 }}>
            {lobby.state!.invitations.map(x => <InvitedPlayer player={x} key={x.toSummonerId}/>)}
        </ScrollView>

        <SectionHeader>SUGGESTED</SectionHeader>
        <ScrollView alwaysBounceVertical={false} style={{ flex: 1 }}>
            {suggestedPlayers.suggestions.map(x => <InvitablePlayer suggestion={x} key={x.summonerId}/>)}
        </ScrollView>
    </>;
});
