...
const MemberSpells = ({ member }: { member: ChampSelectMember }) => <SummonerSpells>
...
const SummonerStatus = ({ member }: { member: ChampSelectMember }) => {
    ...
    return <SummonerStatusContainer>
...
const Member = observer(({ member, first = false }: { member: ChampSelectMember, first?: boolean }) => {
    ...
    return <MemberContainer first={first} resizeMode="cover">
        {/* Background image. */}
        <ChampionBackground {...champSelect.members.getMemberBackground(member) as any} />

        {/* Background style */}
        {flameStyle !== "" &&
        <AnimatedFlameBackground size={MEMBER_HEIGHT} style={{ opacity: 0.6 }} colors={FLAME_COLORS[flameStyle]}
                                 duration={5000}/>}
...
const Team = ({ members, name }: { members: ChampSelectMember[], name: string }) => <TeamContainer>
    <TeamName>{name}</TeamName>
    {members.map((x, i) => <Member member={x} key={x.cellId} first={i === 0}/>)}
</TeamContainer>;

export default function Members() {
    return <Container alwaysBounceVertical={false}>
